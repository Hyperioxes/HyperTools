function HT_processLoad(trackerLoad)
	...
	if not HT_checkIfItemSetsEquipped(trackerLoad.itemSets) then return false end
...
function processTracker(parentControl,originalValues)
	...
	if processLoad(originalValues.load) then
		...
		for _,condition in pairs(originalValues.conditions) do
			if operators[condition.operator](conditionArgs1[condition.arg1](originalValues),condition.arg2) then conditionResults[condition.result](tracker,condition.resultArguments) end
		end
